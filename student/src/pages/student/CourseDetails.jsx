  ...
  return  courseData? (
    ...
    <div className='absolute top-0  left-0 w-full  h-[500px] -z[-1] bg-gradient-to-b from-cyan-100/70'></div>
     {/*left column */}
      ...
      <h1 className="text-[36px] md:text-[44px] font-semibold text-gray-800">{courseData.courseTitle}</h1>
      ...
      {/*review and ratings */}
       <div className='flex items-center space-x-2 pt-3 pb-1 text-sm'>
              ...
              <p className='text-sm'>Course by <span className='text-blue-600 underline'>Bhavesh Girase</span></p>
                      ...
                      <div className={`overflow-hidden transition-all duration-300 ${openSections[index]? 'max-h-96': 'max-h-0'}`}>
                        <ul className='list-disc md:pl-10 pl-4 pr-4 py-2 text-gray-600 border-t border-gray-300'>
                          {chapter.chapterContent.map((lecture,index)=>(
                            <li key={index} className='flex gap-3 h-full m-3 '>
                              <img src={assets.play_icon} alt="play icon" className='w-4 h-4 mt-1 ' />
                              <div className='flex items-center justify-between w-full text-gray-800 text-xs md:text-default'>
                                <p className='flex text-sm'>{lecture.lectureTitle}</p>
     ...
     {/*right column */}
      <div className='max-w-course-card z-10 shadow-custom-card rounded-md md:rounded overflow-hidden bg-white min-w-[300px] sm:min-w-[420px]'>
        { playerData ? 
        ...
        <div className='p-5'>
          ...
          <button className='md:mt-6 mt-4 w-full py-3 rounded bg-blue-600 hover:bg-blue-800 text-white font-medium'>{isAlreadyEnrolled ? 'Already Enrolled':'Enroll Now'} </button>
          <div className='pt-6 '>
            ...
            <ul className='ml-4 pt-2 text-sm md:text-default list-disc text-gray-500'>
              ...
              <li>Downloads resources and source code.</li>
